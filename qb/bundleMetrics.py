...
def centroid_mean(bundle):
	return sum(bundle)/len(bundle)

def centroid_mean_align(bundle):
	fibers = align_fibers(bundle)
	return sum(fibers)/len(fibers)

def fiber_dist_avg(f1,f2):
	distances = []
	for i in range(len(f1)):
		distances.append(np.linalg.norm(f1[i]-f2[i]))
	return np.mean(distances)

def fiber_dist_max(f1,f2):
	distances = []
	for i in range(len(f1)):
		distances.append(np.linalg.norm(f1[i]-f2[i]))
